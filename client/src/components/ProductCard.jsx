...
const modeColors = {
  rental: { bg: '#D23AB0', label: 'location' },
  sale: { bg: '#7B61FF', label: 'achat' },
  both: { bg: '#AE59CE', label: 'loc + achat' },
}

export default function ProductCard({ product }) {
  const { t } = useTranslation()

  const mode = modeColors[product.mode] || modeColors.rental
  ...
  return (
    <Link
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow = '0 16px 48px rgba(210,58,176,0.12)'
    ...
    >
      ...
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: '4/5', backgroundColor: '#1a1a1a' }}>
        {product.images?.[0] ? (
          <img
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg className="w-16 h-16" fill="none" stroke="#333" strokeWidth="0.8" viewBox="0 0 24 24" role="img" aria-label="No image available">
        ...
        {product.pricing_type !== 'suggestion' && (
          <div className="absolute bottom-3 right-3 px-3 py-1.5 backdrop-blur-sm"
            style={{ backgroundColor: 'rgba(13,13,13,0.8)', border: '1px solid rgba(210,58,176,0.2)' }}>
            <span className="text-sm font-bold" style={{ fontFamily: 'Outfit, sans-serif', color: '#FFFFFF' }}>
              {product.price_per_day} {t('catalog.currency')}
            </span>
            <span className="text-xs ml-1" style={{ fontFamily: 'Outfit, sans-serif', color: '#666' }}>
        ...
        {product.pricing_type === 'suggestion' && (
          <div className="absolute bottom-3 right-3 px-3 py-1.5 backdrop-blur-sm"
            style={{ backgroundColor: 'rgba(13,13,13,0.8)', border: '1px solid rgba(123,97,255,0.3)' }}>
            <span className="text-xs font-semibold" style={{ fontFamily: 'Outfit, sans-serif', color: '#7B61FF' }}>
              Proposez votre prix
            </span>
          </div>
        )}
        ...
        <div className="absolute top-3 right-3 px-2.5 py-1"
          style={{ background: mode.bg, color: '#fff' }}>
      ...
      <div className="p-5">
        ...
        {product.product_ratings?.[0] && (
          <div className="flex items-center gap-1.5 mb-2">
            <svg width="12" height="12" fill="#D23AB0" stroke="#D23AB0" strokeWidth="1" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
            </svg>
            <span className="text-xs font-medium" style={{ color: '#666', fontFamily: 'Outfit, sans-serif' }}>
        ...
        <div className="mt-1">
          {product.pricing_type === 'suggestion' ? (
            <p className="text-xs font-medium" style={{ color: '#7B61FF', fontFamily: 'Outfit, sans-serif' }}>
              Proposez votre prix
            </p>
          ) : product.pricing_type === 'negotiable' ? (
            <div className="flex items-center gap-2">
              <p className="text-sm font-medium" style={{ color: '#A0A0A0', fontFamily: 'Outfit, sans-serif' }}>
            ...
            <div className="flex items-center gap-3">
              ...
              {product.price_purchase && (product.mode === 'sale' || product.mode === 'both') && (
                <p className="text-sm font-bold" style={{ color: '#D23AB0', fontFamily: 'Outfit, sans-serif' }}>
        ...
        {/* Location */}
        {product.location && (
          <p className="text-xs flex items-center gap-1 mt-2" style={{ color: '#444', fontFamily: 'Outfit, sans-serif' }}>
