  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-3xl mb-8 font-bold" style={{ color: 'var(--text-primary)' }}>
        {t('nav.cart')}
        <span className="text-sm ml-3 font-normal" style={{ color: 'var(--text-muted)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            return (
              <div key={item.key} className="p-5 transition-all"
                ...
                <div className="flex gap-4">
                  {/* Image */}
                  <div className="w-20 h-20 flex-shrink-0 overflow-hidden" style={{ backgroundColor: 'var(--bg)', borderRadius: '8px' }}>
                    {item.images?.[0] ? (
                      <img src={item.images[0]} alt={item.name} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                        <svg className="w-8 h-8" fill="none" stroke="var(--border)" strokeWidth="1" viewBox="0 0 24 24">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <span className="inline-block mt-1 px-2 py-0.5 text-xs font-semibold uppercase tracking-wider"
                          style={{
                            background: item.mode === 'rental' ? 'var(--accent-gradient)' : 'var(--accent-gradient-subtle)',
                    ...
                    {item.mode === 'rental' && (
                      <div className="mt-2 flex items-center gap-2">
                        <input type="date" value={item.startDate || ''} min={new Date().toISOString().split('T')[0]}
                          ...
                          style={{ backgroundColor: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)', borderRadius: '6px' }}
                        />
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <button onClick={() => updateQuantity(item.key, item.quantity - 1)}
                          ...
                          style={{ border: '1px solid var(--border)', color: 'var(--text-primary)', borderRadius: '8px', cursor: 'pointer' }}>−</button>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 p-6" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px' }}>
            ...
            <div className="space-y-3 mb-5">
              {items.map((item) => (
                <div key={item.key} className="flex justify-between text-sm">
                  ...
                  <span style={{ color: 'var(--text-primary)' }}>{getItemTotal(item).toFixed(2)} TND</span>
                </div>
              ))}
            </div>
