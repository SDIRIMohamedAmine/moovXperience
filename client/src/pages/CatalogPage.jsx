...
export default function CatalogPage() {
  ...
  const FilterSidebar = () => (
    <div className="space-y-6">
      {/* Search */}
      <div>
        <label className="block text-xs uppercase tracking-wider font-medium mb-3" style={{ color: 'var(--text-muted)', fontFamily: 'Outfit, sans-serif' }}>
      ...
      {hasActiveFilters && (
        <button onClick={clearFilters}
          ...
          style={{ border: '1px solid var(--border-light)', color: 'var(--text-muted)', fontFamily: 'Outfit, sans-serif', cursor: 'pointer' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="lg:hidden mb-4">
          <button onClick={() => setSidebarOpen(!sidebarOpen)}
            ...
            style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontFamily: 'Outfit, sans-serif', cursor: 'pointer' }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs uppercase tracking-wider font-medium hidden sm:block" style={{ color: 'var(--text-muted)', fontFamily: 'Outfit, sans-serif' }}>
                  {t('catalog.sort_by')}
                </span>
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
                  className="px-3 py-2 text-sm cursor-pointer"
                  style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-primary)', fontFamily: 'Outfit, sans-serif' }}>
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-lg mb-2 font-bold" style={{ fontFamily: 'Outfit, sans-serif', color: 'var(--text-primary)' }}>
                  {t('catalog.no_results')}
                </p>
                <p className="text-sm mb-4" style={{ color: 'var(--text-muted)', fontFamily: 'Outfit, sans-serif' }}>
                  {t('catalog.no_results_desc')}
                </p>
                {hasActiveFilters && (
                  <button onClick={clearFilters}
                    className="px-6 py-2 text-sm font-medium"
                    style={{ border: '1px solid var(--accent)', color: 'var(--accent)', fontFamily: 'Outfit, sans-serif', cursor: 'pointer' }}>
                    {t('catalog.clear_filters')}
                  </button>
                )}
              </div>
